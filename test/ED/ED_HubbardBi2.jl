...
function observables(spacial_dims::NTuple{DIMS,Int64},num_species::Int64, t::Float64, U::Float64,μ::Float64
    ...
    if length(spacial_dims)==2 && spacial_dims[2]==1
        spacial_dims=spacial_dims=(spacial_dims[1],) #(spacial_dims[1]) #
    end
    num_spin=2
        ...
        for i in 1:b #runs over βs
            ...
            if !isempty(taus)                
                ...
                for i in 1:length(Nₘ)
                     mod(i,num_species)==0 ? Nₘ[i]=Nₙ[i]+1 : Nₘ[i]=Nₙ[i]
                end
                Nₘ = (Nₘ...,)
                #(Nₙ[1]+1,Nₙ[2],Nₙ[3]+1,Nₙ[4])
                Dₘ =[]
                Uₘ=[]
                if Nₘ in Nₙs
            ...
            ds=findall(ones(spacial_dims).!=0)
          #  for   xt in 1:spacial_dims[1]
            #    for yt  in 1:spacial_dims[2]
            for r in ds
                r0=ds[1]
                SC_corrs[r,i]+=SC_corr_obs(spacial_dims,CartesianIndex(r0) , CartesianIndex(r) , Nₙ , Dₙ , Uₙ, β)
                if !isempty(taus) && !isempty(Dₘ)
                    sw_tau[r,:,i]+=SC_corr_sw_tau_obs(spacial_dims,num_species,CartesianIndex(r0),CartesianIndex(r) 
                    , Nₙ , Dₙ  , Uₙ , Nₘ , Dₘ  , Uₘ , β,taus ) 
                    if length(spacial_dims)>1
                        dw_tau[r,:,:,:,i]+=SC_corr_dw_tau_obs(spacial_dims,num_species,CartesianIndex(r0),CartesianIndex(r)
                        , Nₙ , Dₙ  , Uₙ , Nₘ , Dₘ  , Uₘ , β,taus ) 
                    end
                end
            end
    ...
    return partitions, energys./partitions, numbers./partitions,num_species*SC_corrs, num_species*sw_tau, num_species*dw_tau
end
...
function hamiltonian_operator(state::Array{Int8,DIMS2} ,  t::Float64,U::Float64,μ::Float64) where {DIMS2}
    ...
    diag_term = U*sum( sum(state[ntuple(k->:,spacial_dims+1)...,1].-state[ntuple(k->:,spacial_dims+1)...,2] ,dims=spacial_dims+1 ).^2 )/2/num_species #interaction term
